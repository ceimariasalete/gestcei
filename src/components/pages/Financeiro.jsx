...
export default function Financeiro() {
  ...
  const [formConta, setFormConta] = useState({ id: null, saldo_inicial: "" });
  ...
  const saldoInicial = useMemo(() => contas.reduce((sum, c) => sum + Number(c.saldo_inicial || 0), 0), [contas]);
  ...
  // ── Salvar saldo da conta ─────────────────────────────────
  async function salvarSaldoConta() {
    if (!formConta.id) return;
    setLoading(true);
    const val = parseFloat(formConta.saldo_inicial) || 0;
    const { error } = await supabase.from("fin_contas").update({ saldo_inicial: val }).eq("id", formConta.id);
    if (error) {
      showMsg("Erro ao atualizar saldo: " + error.message);
    } else {
      showMsg("Saldo inicial atualizado!");
      await log("fin_contas", "UPDATE", formConta.id, `Atualizou saldo inicial para ${val}`, null, { saldo_inicial: val });
      setModal(null);
      carregar();
    }
    setLoading(false);
    ...
    <div style={{ padding: "clamp(0.75rem,3vw,1.5rem)", maxWidth: 1200, margin: "0 auto" }}>
      ...
      {modal === "importar" && (
        <ImportadorExtrato 
          ...
          onImportSuccess={() => {
            setModal(null);
            carregar();
            showMsg("Lançamentos importados com sucesso!");
          }} 
        />
      )}
      ...
      {modal === "receita" && (
        <Modal title={editItem ? "Editar receita" : "Nova receita"} onClose={() => setModal(null)}>
          {/* Ocultado checkbox legado de Saldo Inicial */}
      ...
      {/* Modal Editar Saldo Conta */}
      {modal === "conta_saldo" && (
        <Modal title="Editar Saldo Inicial" onClose={() => setModal(null)}>
          <Input label="Saldo Inicial (R$)" type="number" step="0.01" value={formConta.saldo_inicial} onChange={e => setFormConta(f => ({ ...f, saldo_inicial: e.target.value }))} />
          <AlertBar>Este valor define o saldo inicial da conta. Ele será somado com as receitas e deduzido pelas despesas da conta.</AlertBar>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
            <Btn onClick={() => setModal(null)}>Cancelar</Btn>
            <Btn variant="primary" onClick={salvarSaldoConta} disabled={loading}>{loading ? "Salvando..." : "Salvar"}</Btn>
          </div>
        </Modal>
      )}
