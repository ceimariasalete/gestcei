...
export default function Pedagogico() {
  ...
  function abrirAdd() {
    setForm({ nome: "", categoria: CATEGORIAS_PEDAGOGICO[0], quantidade: 0, unidade: "un", validade: "", estoque_minimo: 1, observacao: "" });
    setEditItem(null);
    setModal("form");
  }
  ...
  async function salvarItem() {
    ...
    if (editItem) {
      ...
      await log("itens_estoque", "UPDATE", editItem.id, `Editou item pedagogico "${form.nome}"`, editItem, form);
    } else {
      const { data } = await supabase.from("itens_estoque").insert({ ...form, tipo: "pedagogico" }).select().single();
      await log("itens_estoque", "INSERT", data?.id, `Adicionou "${form.nome}" ao estoque pedagogico`, null, form);
    }
    setModal(null);
    setLoading(false);
    carregarItens();
  ...
  async function excluirItem(item) {
    ...
    await log("itens_estoque", "DELETE", item.id, `Excluiu item pedagogico "${item.nome}"`, item, null);
    carregarItens();
  ...
  async function registrarMov() {
    ...
    await log("movimentacoes", "INSERT", item.id,
      `${movForm.tipo === "entrada" ? "Entrada" : "Saida"} de ${qtd} ${item.unidade} - "${item.nome}" (pedagogico)`,
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <PageHeader title="Material Pedagogico" subtitle="Controle de materiais e recursos educativos">
      ...
      <div style={{ marginBottom: 24 }}>
        <Table headers={["Item", "Categoria", "Quantidade", "Validade", "Status", ""]}>
          ...
          {itensFiltrados.length === 0 && <EmptyRow colSpan={6} message="Nenhum material encontrado." />}
        </Table>
      </div>
        ...
        {movs.slice(0, 10).map((m) => (
          <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "0.5px solid #f0f0f0", fontSize: 13 }}>
            <span style={{ padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 500,
              ...
              color: m.tipo === "entrada" ? "#27500A" : "#791F1F" }}>
              {m.tipo}
            </span>
      ...
      {modal === "form" && (
        <Modal title={editItem ? "Editar item" : "Novo material pedagogico"} onClose={() => setModal(null)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <Input label="Nome do material" value={form.nome || ""} onChange={(e) => setForm({ ...form, nome: e.target.value })} />
            <SelectField label="Categoria" value={form.categoria || ""} onChange={(e) => setForm({ ...form, categoria: e.target.value })}>
              {CATEGORIAS_PEDAGOGICO.map((c) => <option key={c}>{c}</option>)}
            </SelectField>
            ...
            <Input label="Validade (se aplicavel)" type="date" value={form.validade || ""} onChange={(e) => setForm({ ...form, validade: e.target.value })} />
          </div>
          <Input label="Observacao (opcional)" value={form.observacao || ""} onChange={(e) => setForm({ ...form, observacao: e.target.value })} placeholder="Ex: Marca, cor, tamanho..." />
      ...
      {modal === "mov" && (
        <Modal title="Registrar movimentacao" onClose={() => setModal(null)}>
          ...
          <Input label="Observacao" value={movForm.observacao} onChange={(e) => setMovForm({ ...movForm, observacao: e.target.value })} placeholder="Ex: Usado na turma M1A, compra mensal..." />
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 8 }}>
            <Btn onClick={() => setModal(null)}>Cancelar</Btn>
