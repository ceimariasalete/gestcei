...
import { fmt, statusItem, exportarCSV } from "../../lib/helpers";
...
export default function Escritorio() {
  ...
  async function salvarItem() {
    ...
    if (editItem) {
      ...
      const { data } = await supabase.from("itens_estoque").insert({ ...form, tipo: "escritorio" }).select().single();
  ...
  async function registrarMov() {
    ...
    if (movForm.tipo === "saida" && item.quantidade < qtd) { showMsg("Erro: Quantidade insuficiente!"); return; }
    setLoading(true);
    ...
    await log("movimentacoes", "INSERT", item.id, `${movForm.tipo === "entrada" ? "Entrada" : "Saida"} de ${qtd} ${item.unidade} - "${item.nome}" (escritorio)`, { quantidade: item.quantidade }, { quantidade: novaQtd });
  ...
  return (
    <div style={{ padding: "1.5rem", maxWidth: 1100, margin: "0 auto" }}>
      ...
      <PageHeader title="Material de Escritorio" subtitle="Controle de materiais administrativos">
        ...
        <Btn variant="primary" onClick={() => { setForm({ nome: "", categoria: CATEGORIAS_ESCRITORIO[0], quantidade: 0, unidade: "un", validade: "", estoque_minimo: 1, observacao: "" }); setEditItem(null); setModal("form"); }}>+ Novo item</Btn>
      </PageHeader>
